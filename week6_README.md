...
⚠️ MOST COORDINATION-HEAVY WEEK
...
- Person A: is cnn.py final and committed? ✅ / ❌
- Person B: is vit.py and fusion.py final? ✅ / ❌
- Person C: can you start model.py on Wednesday? ✅ / ❌

If files are not ready by Wednesday, Person C uses placeholder classes that return random tensors of the right shape — do not block on missing files.

---

## Tasks by Person

### Person A — Training Loop
...
3. Real loop structure: for epoch in range(max_epochs): train one epoch → compute val F1 → check early stopping → save checkpoint if improved
...
6. Early stopping: if val_F1 has not improved for 15 consecutive epochs → break
...
9. Test run: 2 epochs on 100 images only — confirm loss decreases, W&B updates, checkpoint saves
...
### Person B — Cross-Attention Fusion
**File:** `ml/src/fusion.py`

**Step by step:**
1. Use nn.MultiheadAttention — do NOT implement attention from scratch
...
4. Add FFN after second attention: Linear(D, D*4) → GELU → Dropout → Linear(D*4, D)
5. Forward: (cnn_emb [B,D], vit_emb [B,D], meta_emb [B,D]) → fused [B,D]
6. Note: MultiheadAttention expects [B,1,D] for single-vector inputs — unsqueeze and squeeze
7. Test with random tensors: 3 random [4,512] tensors → should output [4,512]
8. Person C needs this file by Wednesday — commit it by Tuesday night

**Why this matters:** Cross-attention is smarter than simple concatenation. When the model sees an ambiguous lesion, it can attend to patient metadata (age, location) to resolve the ambiguity — mimicking how a real dermatologist integrates visual and clinical information. Prior work (Restrepo et al.) confirmed cross-attention consistently outperforms all simpler fusion strategies.

---

### Person C — Focal Loss + Full Model Assembly
**File:** `ml/src/loss.py + ml/src/model.py`

**Step by step:**
1. loss.py first: FocalLoss class with gamma=2.0, label_smoothing=0.1
2. Verify focal loss: easy example (model very confident) → focal loss << crossentropy. Hard example → similar. If this is not the case, formula is wrong.
3. model.py: SkinFuseNet class takes CNN branch, ViT branch, BERT branch, fusion as constructor args
4. Forward: (image [B,3,256,256], input_ids [B,128], attention_mask [B,128]) → logits [B,7]
...
**Why this matters:** Model assembly is the most coordination-heavy task — it combines work from all three people. Getting the constructor arguments right means Person A's training loop can swap model configurations cleanly for the ablation study.
...
- [ ] Real loop structure: for epoch in range(max_epochs): train one epoch → compute v...
- [ ] Optimiser: torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-2)
- [ ] Scheduler: CosineAnnealingLR with 10 epoch warmup

### Person B
- [ ] Use nn.MultiheadAttention — do NOT implement attention from scratch
...
- [ ] Add FFN after second attention: Linear(D, D*4) → GELU → Dropout → Linear(D*4, D)
- [ ] Forward: (cnn_emb [B,D], vit_emb [B,D], meta_emb [B,D]) → fused [B,D]

### Person C
- [ ] loss.py first: FocalLoss class with gamma=2.0, label_smoothing=0.1
- [ ] Verify focal loss: easy example (model very confident) → focal loss << crossentr...
- [ ] model.py: SkinFuseNet class takes CNN branch, ViT branch, BERT branch, fusion as...
- [ ] Forward: (image [B,3,256,256], input_ids [B,128], attention_mask [B,128]) → logi...
...
**Fix:** Check that ml/checkpoints/ folder exists. Git does not track empty folders — create a .gitkeep file inside it.
...
*SkinFuseNet · Week 6 · All 3 team members*